...
> **v2.3.4** ｜ 6255 数据文件 / 8 个技能 / 74 份深度扩充 / 10 模型适配卡
...
├── data/                # 数据文件 (6255个, 含 36/37/38/39 实测报告系列)
│   ├── skill_*.md       # 15个知识文档
...
| 扩充 | 51 | 深度精读扩充卷（剧情/角色/系统/风格/模型适配研究/战斗关卡汇总；目录含 36/37/38/39 实测报告系列） |
...
| 律的调校 | 1 | 调律系统 |
| 局外演绎 | 1 | 塞梅尔维斯叙事 |
| 合计 | 6255 | 2026-08-11 实测口径（含战斗关卡1836/其他2236等）_*.txt 16个、3份实测报告（全量6235） |
...
`data/扩充/` 为深度精读扩充卷（74个文件），由各"深读官"对原始数据逐文件精读整理而成，定位为知识文档（skill_*.md）的深度补充。引用规范：扩充文档每条信息均标注原始出处文件，与已有文档冲突处如实标注、不擅自定对错。
